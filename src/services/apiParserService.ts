...
export function parseApiJson(raw: string, meta?: any[]): ExtractedData {
  ...
  for (let i = 0; i < rawPlaces.length; i++) {
    ...
    const iconSlug: string = asString(placeMeta.__icon) ?? "";
    const typeLabel: string = asString(placeMeta.__type) ?? "";
    const gcid: string = asString(placeMeta.__gcid) ?? "";  // e.g. "gcid:ramen_restaurant"
